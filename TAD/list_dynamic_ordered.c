#include "list_dynamic_ordered.h"
...
{
    tPosL p;
    if (!createNode(&p)) //si la lista está llena
        return false;
    else
    {
        p->Item = item;
        p->sig = LNULL;
        if (isEmptyList(*L)) //se inserta al final
            *L = p;
        else if (item < (*L)->Item)
        {
            p->sig = *L;
        ...
        {
            tPosL i;
            for (i = *L; i->sig->Item<item; i = i->sig); //i queda en el primer elemento mayor que item, tendrá que añadirse antes
